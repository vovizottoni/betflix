<div class="dropdown-end dropdown ml-1">
  <div class="form-control">
    <label class="input-group">
      <span class="px-0 lg:px-4">@livewire('header.balance')</span>
      <label
        tabindex="0"
        class="remove-focus btn px-2 lg:px-4 border-none bg-red-700 hover:bg-red-800"
        style="border-radius: 0px 3px 3px 0px;"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="w-4 text-white"
          viewBox="0 0 512 512"
        >
          <path
            fill="currentColor"
            d="M64 32C28.7 32 0 60.7 0 96V416c0 35.3 28.7 64 64 64H448c35.3 0 64-28.7 64-64V192c0-35.3-28.7-64-64-64H80c-8.8 0-16-7.2-16-16s7.2-16 16-16H448c17.7 0 32-14.3 32-32s-14.3-32-32-32H64zM416 336c-17.7 0-32-14.3-32-32s14.3-32 32-32s32 14.3 32 32s-14.3 32-32 32z"
          />
        </svg>
      </label>
    </label>
  </div>
  @livewire('player.balance')
</div>
