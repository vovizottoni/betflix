...
class Bonus3
{
    ...
    public function canBePaid($user)
    {
        $qty = $user->getDepositsQty();
        if ($qty >= 2) {
            return true;
        }
        return false;
    ...
    function betLose($accountId, $amount_, $balance_used___, $fungamessGamesGainsId_BetPlacing)
    {
        //###############################----------------------------------------3333333
        //###############################----------------------------------------3333333
        //###############################----------------------------------------3333333
        ...
        $checkBet = new \StdClass();
        $checkBet->accounts_id = $accountId;
        $checkBet->balance_used = $balance_used___;
        $checkBet->id = null;
        $checkBet->amount = $amount_;
        ...
        $canBePaid = $this->canBePaid($userRed);
        if (!$canBePaid) {
            return true;
        }
        $userB = $userRed;
        ...
        if ($userB->user_id && $userB->invite_code && empty($userB->bonus3_nivelhierarquico)) {
            ...
            if ($afiliado->bonus3_nivelhierarquico == 'master' || $afiliado->bonus3_nivelhierarquico == 'supervisor' || $afiliado->bonus3_nivelhierarquico == 'gerente' || $afiliado->bonus3_nivelhierarquico == 'subgerente') {
                ...
                if ($afiliado->bonus3_nivelhierarquico == 'master' && is_numeric($afiliado->bonus3_percentual)) {
                    ...
                    Bonus::create(['accounts_id' => $primeira_account->id, 'group_id' => $fungamessGamesGainsId_BetPlacing, 'bets_id' => $checkBet->id, 'amount' => $bonusFINAL, 'group_tipo' => '3', 'users_id_gerador_do_bonus' => $userB->id, 'bonus3_processado' => 'n', 'bonus3_semanapagamento' => $semanaPgto, 'bonus3_sinal' => '+']);
        ...
        if ($userB->bonus3_nivelhierarquico == 'master' || $userB->bonus3_nivelhierarquico == 'supervisor' || $userB->bonus3_nivelhierarquico == 'gerente' || $userB->bonus3_nivelhierarquico == 'subgerente') {
            ...
            if ($userB->bonus3_nivelhierarquico == 'master') {
                ...
                // grava esse bonus no BD (para histórico)
                Bonus::create(['accounts_id' => $primeira_account->id, 'group_id' => $fungamessGamesGainsId_BetPlacing, 'bets_id' => $checkBet->id, 'amount' => $bonusFINAL, 'group_tipo' => '3', 'users_id_gerador_do_bonus' => $supervisor->id, 'bonus3_processado' => 'n', 'bonus3_semanapagamento' => $semanaPgto, 'bonus3_sinal' => '+']);
                ...
                // grava esse bonus no BD (para histórico)
                Bonus::create(['accounts_id' => $primeira_account->id, 'group_id' => $fungamessGamesGainsId_BetPlacing, 'bets_id' => $checkBet->id, 'amount' => $bonusFINAL, 'group_tipo' => '3', 'users_id_gerador_do_bonus' => $gerente->id, 'bonus3_processado' => 'n', 'bonus3_semanapagamento' => $semanaPgto, 'bonus3_sinal' => '+']);
                ...
                // grava esse bonus no BD (para histórico)
                Bonus::create(['accounts_id' => $primeira_account->id, 'group_id' => $fungamessGamesGainsId_BetPlacing, 'bets_id' => $checkBet->id, 'amount' => $bonusFINAL, 'group_tipo' => '3', 'users_id_gerador_do_bonus' => $subgerente->id, 'bonus3_processado' => 'n', 'bonus3_semanapagamento' => $semanaPgto, 'bonus3_sinal' => '+']);
    ...
    function betWin($accountId, $lucro___, $balance_used___, $fungamessGamesGainsId_BetPlacing)
    {
        //contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3
        //contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3
        //contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3
        //contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3
        //contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3 contra-peso-bonus3
        ...
        //Bonus3 contra-peso: todos greens de usuario que tenha sido indicado por um usuario-afiliado (user_id) que pertenca a alguma hierarquia de bonus3, PROCESSA bonus para esta Hierarquia (CONTRA-PESO)
        //Semanalmente é rodado o comando de console p/ fazer a diferenca do somatorio de greens e reds, e ver se o afiliado tem direito a receber seu bonus3


        // --- CODIGO ABAIXO APROVEITADO DE BONUS3 RED ---
        // --- CODIGO ABAIXO APROVEITADO DE BONUS3 RED ---
        ...
        //este usuário que ganhou a aposta, foi indicado por algum afiliado?
        $accountRed = Account::where([['id', '=', $checkBet->accounts_id]])->first();

        //busca usuario que tomou green
        $userRed = User::where([['id', '=', $accountRed->users_id]])->first();
        $canBePaid = $this->canBePaid($userRed);
        if (!$canBePaid) {
            return true;
        ...
        //verifica se o USER que tomou green foi indicado por algum usuario-AFILIADO (user_id)
        if ($userB->user_id && $userB->invite_code && empty($userB->bonus3_nivelhierarquico)) {
            ...
            if ($afiliado->bonus3_nivelhierarquico == 'master' || $afiliado->bonus3_nivelhierarquico == 'supervisor' || $afiliado->bonus3_nivelhierarquico == 'gerente' || $afiliado->bonus3_nivelhierarquico == 'subgerente') {
                ...
                if ($afiliado->bonus3_nivelhierarquico == 'master' && is_numeric($afiliado->bonus3_percentual)) {
                    ...
                    bcscale(2);
                    $percentB = bcdiv($afiliado_bonus3_percentual_string, '100.0');
                    $bonusFINAL = bcmul($percentB, $lucro___);
                    ...
                    // grava esse bonus no BD (para histórico e calculo do commando semanal de pagamento)
                    Bonus::create(['accounts_id' => $primeira_account->id, 'group_id' => $fungamessGamesGainsId_BetPlacing, 'bets_id' => $checkBet->id, 'amount' => $bonusFINAL * -1, 'group_tipo' => '3', 'users_id_gerador_do_bonus' => $userB->id, 'bonus3_processado' => 'n', 'bonus3_semanapagamento' => $semanaPgto, 'bonus3_sinal' => '-']);
        ...
        if (($userB->bonus3_nivelhierarquico == 'master' || $userB->bonus3_nivelhierarquico == 'supervisor' || $userB->bonus3_nivelhierarquico == 'gerente' || $userB->bonus3_nivelhierarquico == 'subgerente') && ($userB->user_id && $userB->invite_code)) {
            ...
            if ($userB->bonus3_nivelhierarquico == 'master') {
                ...
                // grava esse bonus no BD (para histórico)
                Bonus::create(['accounts_id' => $primeira_account->id, 'group_id' => $fungamessGamesGainsId_BetPlacing, 'bets_id' => $checkBet->id, 'amount' => $bonusFINAL * -1, 'group_tipo' => '3', 'users_id_gerador_do_bonus' => $supervisor->id, 'bonus3_processado' => 'n', 'bonus3_semanapagamento' => $semanaPgto, 'bonus3_sinal' => '-']);
                ...
                // grava esse bonus no BD (para histórico)
                Bonus::create(['accounts_id' => $primeira_account->id, 'group_id' => $fungamessGamesGainsId_BetPlacing, 'bets_id' => $checkBet->id, 'amount' => $bonusFINAL * -1, 'group_tipo' => '3', 'users_id_gerador_do_bonus' => $gerente->id, 'bonus3_processado' => 'n', 'bonus3_semanapagamento' => $semanaPgto, 'bonus3_sinal' => '-']);
                ...
                // grava esse bonus no BD (para histórico)
                Bonus::create(['accounts_id' => $primeira_account->id, 'group_id' => $fungamessGamesGainsId_BetPlacing, 'bets_id' => $checkBet->id, 'amount' => $bonusFINAL * -1, 'group_tipo' => '3', 'users_id_gerador_do_bonus' => $subgerente->id, 'bonus3_processado' => 'n', 'bonus3_semanapagamento' => $semanaPgto, 'bonus3_sinal' => '-']);
